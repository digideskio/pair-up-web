...
class Users extends React.Component {
  ...
  render() {
    ...
    const matchingUsers = users.reduce((memo, user) => {
      ...
      if (lowerFirst.match(lowerName) || lowerLast.match(lowerName)) {
        memo.push(user);
      }
      return memo;
    }, []);
                  ...
                  <h4 style={{ 'marginTop': '20px' }}>There {pluralize[0]} currently {count} User{pluralize[1]}:</h4>
                  {
                    matchingUsers.map(user => (
                      <li key={user.id} className='list-group-item'>
                        ...
                        {/* check out this empty span... have found way too many of these!! even in line styling is better! */}
                        <Link to={`/users/${user.id}`}>
                          <button className='btn2 btn-info btn-sm' style={{ float: 'right' }}>Edit user</button>
                        </Link>
                      </li>
                    ))
                  }
